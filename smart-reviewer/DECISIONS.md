...
- **Decision**: No unit tests — manual testing via VSCode chat
  - **Context**: Extension deeply integrates with VSCode APIs (chat participant, tree view, secret storage)
  - **Rationale**: Mocking VSCode APIs for unit tests would be fragile and high-maintenance. The extension is best tested manually by installing the VSIX and using `@smart-reviewer` in chat. Future iterations could add integration tests.
  - **Date**: 2025-04-14

- **Decision**: VSIX excluded from git (build artifact)
  - **Context**: `smart-reviewer-0.1.0.vsix` is generated by `vsce package`
  - **Rationale**: Binary artifacts shouldn't be in version control. The build is reproducible from source. `.gitignore` already excludes `*.vsix`.
  - **Date**: 2025-04-14
